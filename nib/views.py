...
from nib.settings import NIB_SW_IMAGE_PATH
from nib.util import delete_nib_image, get_nib_imagelist, send_recv, stop_swupgrade

nib_sw_upgrade_in_process = 0
nib_sw_upgrade_image = ''
...
def upgrade(request, host_id):
    ...
    global nib_sw_upgrade_in_process
    global nib_sw_upgrade_image
    errors = []
    images = get_nib_imagelist()
    image_name = nib_sw_upgrade_image
    ...
    if request.method == 'POST':
        ...
        if 'upgrade_image' in request.POST:
            nib_sw_upgrade_image = request.POST.get('img_name', '')
            """
    ...
    if nib_sw_upgrade_in_process:
        activation = 0
        if nib_sw_upgrade_in_process == 2:
            activation = 1
...
def upgrade_status(request, host_id):
    ...
    if not nib_sw_upgrade_in_process:
...
def reboot(request, host_id):
    ...
    disable_reboot = nib_sw_upgrade_in_process
